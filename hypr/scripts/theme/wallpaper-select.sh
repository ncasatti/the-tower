...
show_folder_contents() {
    ...
    if [[ "$dir" != "$wallDIR" ]]; then
        relative_path="$FOLDER_ICON $(basename "$dir")"
        printf "%s\n" "$BACK_ICON .. (back)"
    fi
    ...
    while IFS= read -r -d '' folder; do
        ...
        if [[ -n "$random_img" ]]; then
            printf "%s\x00icon\x1f%s\n" "$FOLDER_ICON $folder_name" "$random_img"
        else
    ...
    while IFS= read -r -d '' pic_path; do
        ...
        if [[ ! "$pic_name" =~ \.gif$ ]]; then
            printf "%s\x00icon\x1f%s\n" "$IMAGE_ICON $(echo "$pic_name" | cut -d. -f1)" "$pic_path"
        else
            printf "%s\n" "$VIDEO_ICON $pic_name"
        fi
...
set_wallpaper() {
    ...
    "$SCRIPTSDIR/theme/wallpaper-set.sh" "$wallpaper_path"
...
navigate() {
    while true; do
        ...
        clean_choice="$choice"
        if [[ "$choice" == "$FOLDER_ICON"* ]]; then
            clean_choice="${choice#"$FOLDER_ICON "}"
        elif [[ "$choice" == "$IMAGE_ICON"* ]]; then
            clean_choice="${choice#"$IMAGE_ICON "}"
        elif [[ "$choice" == "$VIDEO_ICON"* ]]; then
            clean_choice="${choice#"$VIDEO_ICON "}"
        fi
        ...
        if [[ "$choice" == "$IMAGE_ICON"* ]] || [[ "$choice" == "$VIDEO_ICON"* ]]; then
